#ifndef jpr_RGEX_GFX2D
#define jpr_RGEX_GFX2D
...
public:
    ...
    static void DrawSprite(jpr::Sprite *sprite, jpr::GFX2D::Transform2D &transform);
};
} // namespace jpr

#ifdef jpr_RGEX_GRAPHICS2D
#undef jpr_RGEX_GRAPHICS2D

namespace jpr
{
void GFX2D::DrawSprite(jpr::Sprite *sprite, jpr::GFX2D::Transform2D &transform)
{
    if (sprite == nullptr)
        return;
    ...
    for (float i = sx; i < ex; i++)
    {
        for (float j = sy; j < ey; j++)
        {
            ...
            pge->Draw((int32_t)i, (int32_t)j, sprite->GetRetro((int32_t)(ox + 0.5f), (int32_t)(oy + 0.5f)));
        }
    }
}

jpr::GFX2D::Transform2D::Transform2D()
{
    Reset();
}

void jpr::GFX2D::Transform2D::Reset()
{
    nTargetMatrix = 0;
    nSourceMatrix = 1;
    bDirty = true;
...
void jpr::GFX2D::Transform2D::Multiply()
{
    for (int c = 0; c < 3; c++)
    {
        for (int r = 0; r < 3; r++)
...
void jpr::GFX2D::Transform2D::Rotate(float fTheta)
{
...
void jpr::GFX2D::Transform2D::Scale(float sx, float sy)
{
...
void jpr::GFX2D::Transform2D::Shear(float sx, float sy)
{
...
void jpr::GFX2D::Transform2D::Translate(float ox, float oy)
{
...
void jpr::GFX2D::Transform2D::Perspective(float ox, float oy)
{
    // Construct Translate Matrix
    matrix[2][0][0] = 1.0f;
    matrix[2][1][0] = 0.0f;
...
void jpr::GFX2D::Transform2D::Forward(float in_x, float in_y, float &out_x, float &out_y)
{
...
void jpr::GFX2D::Transform2D::Backward(float in_x, float in_y, float &out_x, float &out_y)
{
...
void jpr::GFX2D::Transform2D::Invert()
{
